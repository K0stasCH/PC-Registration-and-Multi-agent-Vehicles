...
class Scene():
    ...
    def createPCD(self, labels:list[str], cropDownLimits:list[float]=[None,None,-1.2]):
        ...
        if cropDownLimits[0] is not None:
            points3d = points3d[points3d[:,0] > cropDownLimits[0]]
        if cropDownLimits[1] is not None:
            points3d = points3d[points3d[:,1] > cropDownLimits[1]]
        if cropDownLimits[2] is not None:
            points3d = points3d[points3d[:,2] > cropDownLimits[2]]
    ...
    def findcenters(self, _min_points, _eps, className:str='vehicle', visualize:bool=True):
        indxes = self._getPoints_idx(className)
        points3d = np.array(self.pcd.points)[indxes,:]
        ...
        # hdb = HDBSCAN(min_cluster_size=30, min_samples=60, cluster_selection_epsilon=0,
        #               max_cluster_size=None, alpha=1, store_centers="centroid").fit(points3d)
        # labeledCluster = hdb.labels_
        # labeledCluster[hdb.probabilities_<0.99] = -1
        # nodes = hdb.centroids_
        ...
        if visualize:
            ...
            ax[0].scatter(points3d[:,0],points3d[:,1])

            max_label = labeledCluster.max()
            for i in range(0,max_label+1):
                indxes =  np.where(labeledCluster == i)[0]
                ax[1].scatter(points3d[indxes,0], points3d[indxes,1])
